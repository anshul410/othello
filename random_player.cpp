/*
  random_player.cpp
  Scans the board, collects all available moves and plays any move randomly.
  Replace p1 or p2 or both with random_player to make play this code.
*/
#include <iostream>
#include <cstdio>
#include <cstring>
...
public :
  ...
  vector<string> generateAllMoves(int turn) {
    vector<string> moves;
    int row, column;
    for (row = 0; row < 8; row++) {
      for (column = 0; column < 8; column++) {
...
string strategicMove(int turn)
{
  /* play your strategy accordinlgy whether you are white or black
  which is denoted by turn
  */
  /* implement your strategy here......*/
  vector<string> allMoves = currentGame.generateAllMoves(turn);
  return allMoves[random(allMoves.size())];
}
